jQuery(function($) {
	...
	// Form types: 1 = anything else, 2 = TML shortcode or widget, WP Members, Ultimate Membership Pro or Elementor login form
	...
	function tfaShowOTPField(form_type, form, user_can_trust, user_already_trusted) {
		...
		$submit_button.parents('form').first().find('p, .impu-form-line-fr, .tml-field-wrap, .user-pass-wrap, .elementor-field-type-text, .elementor-field-type-submit, .elementor-remember-me').each(function(i) {
		...
		html += '<p class="submit"><input id="tfa_login_btn" class="button button-primary button-large" type="submit" value="' + submit_button_text + '"></p>';
	...
	var tfa_cb = function(e) {
		...
		if ($(e.target).parents('.tml-login, .elementor-widget-login').length > 0 || $(e.target).closest('#wpmem_login').find('form').length > 0 || 'ihc_login_form' == $(e.target).attr('id')) {
			$(e.target).off();
			form_type = 2;
			form = e.target;
		} else {
	...
	// Elementor login form - April 2020
	$('#ihc_login_form').off('submit');
	$('.tml-login form[name="loginform"], .tml-login form[name="login"], #wpmem_login form, form#ihc_login_form').on('submit', tfa_cb);
	if (simba_tfasettings.hasOwnProperty('support_elementor') && simba_tfasettings.support_elementor) {
		$('form.elementor-login').on('submit', tfa_cb);
	}
