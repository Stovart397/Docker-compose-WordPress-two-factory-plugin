jQuery(function($) {
	...
	if($('.woocommerce form.login').length > 0) {
		...
		var tfa_wc_user_field = $(tfa_wc_form).find('#affwp-login-user-login, #affwp-user-login').first();
		var tfa_wc_pass_field = $(tfa_wc_form).find('#affwp-login-user-pass, #affwp-user-pass').first();
		var tfa_wc_submit_btn = $(tfa_wc_form).find('input[type=submit]').first();
	}
